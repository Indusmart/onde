...
def step_to_obj_with_highlight(step_file, face_index, output_obj_file):
    """
    Convert a STEP file to an OBJ file, highlighting a specific face with a different color.

    Parameters:
    step_file (str): Path to the input STEP file.
    face_index (int): Index of the face to highlight (1-based index).
    ...
    if face_index < 1 or face_index > len(faces):
        print(f"Error: Face index {face_index} is out of range. The STEP file contains {len(faces)} faces.")
        return

    # Highlight the specified face
    highlight_face = faces[face_index - 1]  # Convert 1-based index to 0-based
    ...
    with open(output_mtl_file, 'w') as mtl_file:
        ...
        mtl_file.write("Kd 1.0 0.0 0.0\n")  # Red color for highlighted face
        mtl_file.write("\n")
    ...
    with open(output_obj_file, 'w') as obj_file:
        ...
        for face in faces:
            ...
            if face.IsEqual(highlight_face):
...
if __name__ == "__main__":
    if len(sys.argv) != 4:
        print("Usage: python script.py <input_step_file> <face_index> <output_obj_file>")
        sys.exit(1)

    input_step_file = sys.argv[1]
    face_index = int(sys.argv[2])
    output_obj_file = sys.argv[3]

    step_to_obj_with_highlight(input_step_file, face_index, output_obj_file)
